...
export default function Sidebar({ open, setOpen }: { open: boolean; setOpen: (v: boolean) => void }) {
  ...
  // Menu baseado na role do usuário
  const getMenuByRole = () => {
    ...
    const menus = {
      ADMIN: [
        ...
        { label: "Auditoria", path: "/dashboard/auditoria", icon: Shield },
      ],
      RECEPCAO: [
        { label: "Dashboard", path: "/dashboard", icon: LayoutDashboard },
        { label: "Agenda", path: "/dashboard/agenda", icon: Calendar },
      ...
      FINANCEIRO: [
        ...
        { label: "Financeiro", path: "/dashboard/financeiro", icon: DollarSign },
        { label: "Relatórios", path: "/dashboard/relatorios", icon: BarChart3 },
        { label: "Contas", path: "/dashboard/contas", icon: FileText },
      ],
      PROFISSIONAL: [
        { label: "Dashboard", path: "/dashboard", icon: LayoutDashboard },
      ...
      USER: [
        ...
        { label: "Meus Dados", path: "/dashboard/perfil", icon: UserCog },
        { label: "Histórico", path: "/dashboard/historico", icon: History },
      ],
    };

    return menus[role as keyof typeof menus] || menus.USER;
  };
          ...
          <div className="flex items-center gap-3 mb-10">
            ...
            {open && (
              <div>
                <h1 className="text-white font-bold text-lg">Nayane Pimentel</h1>
                <p className="text-white/60 text-xs">Estética Avançada</p>
                <p className="text-white/40 text-xs mt-1 capitalize">{user?.role?.toLowerCase()}</p>
              </div>
            )}
          </div>
